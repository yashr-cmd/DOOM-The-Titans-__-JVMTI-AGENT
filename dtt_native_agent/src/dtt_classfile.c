...
dtt_cf_result_t dtt_cf_inject_guard(const unsigned char *data, jint len,
    ...
    unsigned short target_m_access = 0;
    unsigned short target_m_attr_count = 0;
    ...
    for (i = 0; i < methods_count; i++) {
        ...
        if (is_target) { method_idx_target = i; method_start = m_start; method_end = cursor; target_m_access = m_access; target_m_attr_count = m_attr_count; }
        if (cursor > len) { free(cp.entries); return fail; }
    }
    ...
    /* ---- parse the Code attribute body ---- */
    {
...

        {
            ...
            /* Detect our guard prologue: aload_0; invokestatic; ifeq (8 bytes for
             * cancel styles). Avoid matching unrelated aload_0+invokestatic pairs. */
            if (code_length >= 8 &&
                data[code_off] == 0x2A && data[code_off + 1] == 0xB8 &&
                data[code_off + 4] == 0x99) {
                free(cp.entries);
                return fail;
            }
        }
        ...
        /* locate (if present) the StackMapTable sub-attribute of Code */
        {
            ...
            {
                int is_static = (target_m_access & 0x0008) != 0;
...

                    {
                        ...
                        for (mi = 0; mi < methods_count; mi++) {
                            ...
                            if (mi == method_idx_target) {
                                ...
                                {
                                    unsigned short new_m_attr_count = target_m_attr_count; /* Code count unchanged, only its body grew */
                                    if (!ob_u2(&final_out, new_m_attr_count)) goto bail2;
                                }
                                {
                                    jint r2 = this_m_start + 8;
                                    int a;
                                    for (a = 0; a < target_m_attr_count; a++) {
