...
/* ============================== StackMapTable frame re-encode (subtract 1) ============================== */
...
/* Re-emits the class's ORIGINAL first StackMapTable frame with its
 * offset_delta reduced by exactly 1 (see header/commit notes for why this
 * is correct once we prepend our own leading FULL_FRAME entry). Returns
 * bytes consumed from src, or -1 on an unsupported/inconsistent shape. */
static int reencode_first_frame_minus_one(const unsigned char *src, jint src_len, outbuf_t *ob) {
    unsigned char frame_type;
    if (src_len < 1) return -1;
    frame_type = src[0];

    if (frame_type <= 63) {
        int old_delta = frame_type;
        if (old_delta - 1 < 0) return -1;
        if (!ob_u1(ob, (unsigned char)(old_delta - 1))) return -1;
        return 1;
    }
    if (frame_type >= 64 && frame_type <= 127) {
        ...
        vlen = vti_entry_len(src + 1);
        if (src_len < 1 + vlen) return -1;
        if (!ob_u1(ob, (unsigned char)(64 + (old_delta - 1)))) return -1;
        if (!ob_put(ob, src + 1, vlen)) return -1;
        return 1 + vlen;
    }
    if (frame_type == 247) { /* SAME_LOCALS_1_STACK_ITEM_EXTENDED */
        int old_delta; int vlen;
        if (src_len < 3) return -1;
        old_delta = (int)rd_u2(src + 1);
        if (old_delta - 1 < 0) return -1;
        vlen = vti_entry_len(src + 3);
        if (src_len < 3 + vlen) return -1;
        if (!ob_u1(ob, 247)) return -1;
        if (!ob_u2(ob, (unsigned short)(old_delta - 1))) return -1;
        if (!ob_put(ob, src + 3, vlen)) return -1;
        return 3 + vlen;
    }
    if (frame_type >= 248 && frame_type <= 251) { /* CHOP x3, SAME_FRAME_EXTENDED */
        int old_delta;
        if (src_len < 3) return -1;
        old_delta = (int)rd_u2(src + 1);
        if (old_delta - 1 < 0) return -1;
        if (!ob_u1(ob, frame_type)) return -1;
        if (!ob_u2(ob, (unsigned short)(old_delta - 1))) return -1;
        return 3;
    }
    if (frame_type >= 252 && frame_type <= 254) { /* APPEND */
        int old_delta, n_append, i, off;
        if (src_len < 3) return -1;
        old_delta = (int)rd_u2(src + 1);
        if (old_delta - 1 < 0) return -1;
        ...
        for (i = 0; i < n_append; i++) {
            ...
            if (!ob_put(ob, src + off, vlen)) return -1;
            off += vlen;
        }
        return off;
    }
    if (frame_type == 255) { /* FULL_FRAME */
        int old_delta, n_locals, n_stack, i, off;
        if (src_len < 3) return -1;
        old_delta = (int)rd_u2(src + 1);
        if (old_delta - 1 < 0) return -1;
        n_locals = (int)rd_u2(src + 3);
        off = 5;
        if (!ob_u1(ob, 255)) return -1;
        if (!ob_u2(ob, (unsigned short)(old_delta - 1))) return -1;
        if (!ob_u2(ob, (unsigned short)n_locals)) return -1;
        ...
        if (!ob_u2(ob, (unsigned short)n_stack)) return -1;
        off += 2;
        for (i = 0; i < n_stack; i++) {
            int vlen;
            if (src_len < off + 1) return -1;
    ...
    return -1; /* 128-246 reserved, or malformed */
}
...

                {
                    ...
                    for (a = 0; a < code_attrs_count; a++) {
                        ...
                        if (cp_utf8_eq(data, &cp, nidx, "LineNumberTable")) {
                            ...
                            if (old_count > 0) {
                                int consumed = reencode_first_frame_minus_one(data + src, (jint)(src_end - src), &out);
                                if (consumed < 0) { ob_free(&out); free(cp.entries); return fail; }
                                src += consumed;
                                if (!ob_put(&out, data + src, (jint)(src_end - src))) goto bail; /* remaining frames verbatim */
                            }
