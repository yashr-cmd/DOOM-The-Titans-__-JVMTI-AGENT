...
JNIEXPORT jint JNICALL Agent_OnAttach(JavaVM *vm, char *options, void *reserved) {
    (void)reserved;
    /* Dynamic attach always happens on a fully-initialized VM - VMInit has
     * already fired (or will never fire again), so unblock init-gated logic
     * here or the ModLauncher probe would stay disabled forever. */
