...
static dtt_mutex_t g_patch_once_mutex;
static int g_patch_once_ready = 0;
...
static int patch_once_contains(const char *class_name,
    ...
    if (g_patch_once_ready) {
        DTT_MUTEX_LOCK(&g_patch_once_mutex);
    }
    for (i = 0; i < g_patched_count; i++) {
        if (strcmp(g_patched_keys[i], key) == 0) {
            found = 1;
            break;
        }
    }
    if (g_patch_once_ready) {
        DTT_MUTEX_UNLOCK(&g_patch_once_mutex);
    }
    return found;
}
...
void dtt_vanilla_guard_init(void) {
    if (!g_patch_once_ready) {
        DTT_MUTEX_INIT(&g_patch_once_mutex);
        g_patch_once_ready = 1;
    }
...
dtt_cf_result_t dtt_vanilla_guard_intercept(const char *class_name,
    ...
    dtt_cf_result_t accumulated = { NULL, 0 };
    const unsigned char *work_data = class_data;
    jint work_len = class_data_len;
    int i;
         ...
         * unsupported class shape - all return {NULL,0} which is safe. */
        {
            ...
            if (result.data != NULL) {
                ...
                if (accumulated.data != NULL) {
                    dtt_cf_result_free(&accumulated);
                }
                accumulated = result;
                work_data = accumulated.data;
                work_len = accumulated.len;
            }
