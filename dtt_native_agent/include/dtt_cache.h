 ...
 * sets *out_data/*out_len to point at cache-owned memory (valid until
 * the next dtt_cache_store for the same class, or shutdown - callers
 * must copy the bytes out, e.g. into a jvmtiEnv->Allocate buffer,
 * before returning from the hook). Returns 0 if nothing is cached for
 * that class yet. */
