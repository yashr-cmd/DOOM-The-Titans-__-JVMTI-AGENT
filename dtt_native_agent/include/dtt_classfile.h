 ...
 *   - When prepending a guard prologue of N bytes, a leading FULL_FRAME is
 *     synthesized at offset N, then the original StackMapTable is spliced in.
 *     The original first frame is dropped when it also targeted offset 0;
 *     otherwise its offset_delta is kept unchanged (subsequent frames are
 *     copied verbatim because only the first delta is relative to our frame).
 *
